...
/*
    ...
    resolution is the timer resolution in microseconds.
	The time will advance every resolution ms.

    callback is a function pointer to be called every resolution us.
...
/* 
    Pt_Time() returns the current time in seconds (WAS MS - SIMON WAS HERE).
*/
...
/*
    ...
    duration is the length of the pause in seconds. The true duration 
    of the pause may be rounded to the nearest or next clock tick
    as determined by resolution in Pt_Start().
*/
PMEXPORT void Pt_Sleep(double duration);
